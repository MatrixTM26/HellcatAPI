...
document
    ...
    .addEventListener("click", () => setSidebar(!sidebarOpen));
...
window.addEventListener("resize", () => {
    if (window.innerWidth >= 900) {
        ...
        if (sidebarOpen) overlay.classList.add("open");
    }
});
...
function clearOutput() {
    const el = document.getElementById("output");
    el.textContent = "Waiting for request...";
    el.className = "";
}
...
async function deleteUser(id) {
    const el = document.getElementById("output");
    el.className = "";
    el.textContent = `Deleting user ${id}...`;
    try {
...
async function doMe() {
    const out = document.getElementById("auth-output");
    if (!jwtToken) {
        out.textContent = "Login first to get a token.";
        out.style.color = "#fca5a5";
        return;
    }
    out.style.color = "var(--text-3)";
    out.textContent = "Fetching /auth/me...";
    try {
...
function startStream() {
    ...
    out.innerHTML =
        '<span style="color:var(--text-3)">Connecting to /stream?count=' +
        count +
        "...</span>\n";
    ...
    evtSource.onmessage = function (e) {
        try {
            ...
            if (d.Event === "done") {
                out.innerHTML +=
                    '<span class="stream-done">✔ Stream complete.</span>\n';
                evtSource.close();
                evtSource = null;
            } else {
                out.innerHTML +=
                    '<span class="stream-event">► Event ' +
                    d.Event +
                    "</span>  value=" +
                    d.Value +
                    "  ts=" +
            ...
            out.innerHTML +=
                '<span style="color:#fca5a5">Parse error: ' +
                err.message +
                "</span>\n";
        }
    };
    evtSource.onerror = function () {
        out.innerHTML +=
            '<span style="color:#fca5a5">Connection error.</span>\n';
        if (evtSource) {
            evtSource.close();
            evtSource = null;
        }
...
async function loadUsers() {
    ...
    try {
        ...
        if (rows.length === 0) {
            tbody.innerHTML =
                '<tr><td colspan="4" style="text-align:center;color:var(--text-3);font-size:12px;padding:14px">No users found.</td></tr>';
            return;
        }
        ...
        tbody.innerHTML =
            '<tr><td colspan="4" style="text-align:center;color:#fca5a5;font-size:12px;padding:14px">Failed to load users.</td></tr>';
...
async function loadProducts() {
    ...
    try {
        ...
        if (rows.length === 0) {
            tbody.innerHTML =
                '<tr><td colspan="4" style="text-align:center;color:var(--text-3);font-size:12px;padding:14px">No products found.</td></tr>';
            return;
        }
        tbody.innerHTML = rows
            .map(
        ...
        tbody.innerHTML =
            '<tr><td colspan="4" style="text-align:center;color:#fca5a5;font-size:12px;padding:14px">Failed to load products.</td></tr>';
        if (meta) meta.textContent = "error";
    }
}
